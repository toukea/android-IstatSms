 ...
 */
 final class SmsQuery {
	...
	public static List<Sms> execute(Context context,SmsClause<?> query){
		...
		while(c.moveToNext()){
			...
			sms.setService_center(c.getString(11));
			sms.setLocked(c.getString(12));
			list.add(sms);
		}
		return list;
	}
	public static boolean smsIdExist(Context context,String id){
		Cursor c = context.getContentResolver().query(uriSms,
				new String[]{"_id"},
				"_id=?",
				new String[]{id},
